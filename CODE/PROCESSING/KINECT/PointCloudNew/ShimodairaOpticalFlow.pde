...
// updated and turned into a class for processing 3.0 for USC IML404 by John Carpenter 09/20/2016
...
class ShimodairaOpticalFlow {
  ...
  ShimodairaOpticalFlow(int[] _cam_pixels, int _cam_width, int _cam_height) {
    cam_pixels = _cam_pixels;
  ...
  void pixave(int x1, int y1, int x2, int y2) {
    ...
    if (x1<0) x1=0;
    if (x2>=cam_width) x2=cam_width-1;
    if (y1<0) y1=0;
    if (y2>=cam_height) y2=cam_height-1;

    sumr=sumg=sumb=0.0;
    for (int y=y1; y<=y2; y++) {
      for (int i=cam_width*y+x1; i<=cam_width*y+x2; i++) {
        pix=cam_pixels[i];
        b=pix & 0xFF; // blue
        pix = pix >> 8;
        g=pix & 0xFF; // green
  ...
  void calculateFlow(int[] _cam_pixels) {
    cam_pixels = _cam_pixels;
