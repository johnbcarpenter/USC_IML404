...
3D sensors are a relatively new way for artists and designers to work with 3D data in their environment.  Prior to the release of the Kinect V1, artists and creative coders (I did anyway) typically used RGB or infrared cameras to _visually sense_ the environment. There are still some advantages to RGB/Infrared cameras: _they generally offer more visual detail (higher resolution, provide color) and are more straightforward + quick to set up (powered by USB and/or included w your laptop), and tend to work in all environmental conditions_, but 3D sensors are getting smaller and easier to use all the time (like the [Kinect Azure (V3)](https://github.com/johnbcarpenter/USC_IML404/blob/master/notes_md/computer-vision-kinect.md#kinect-azure-2019), [leap motion](https://github.com/johnbcarpenter/USC_IML404/blob/master/notes_md/computer-vision-kinect.md#leap-motion-2013), and [realsense](https://github.com/johnbcarpenter/USC_IML404/blob/master/notes_md/computer-vision-kinect.md#intel-realsense-2019)).  The big difference between the two types of sensors is that 3D capture provide spatial data from the environment -- and we can work with that data in interesting ways. 

I really like developing with the kinect for a couple of reasons: 1) you can depth filter your camera feed (you can only look for motion <= 10 feet out if you want -- say to remove background noise), and 2) you can work directly with 3D data scans of the environment/body.  Additionally, some of the more recent 3D sensor SDK provide the user with real-time skeletal and hand tracking data.  We'll cover some examples of relevant projects in class today; however, I'd encourage you to do some of your own research as there are a lot of artists doing some pretty incredible things:
...
<sup>^ _dandelion clock_ by johnbcarpenter @ACME. los andeles (2011): a 3D sensor was used to capture optical flow from the environment while removing background noise</sup>
